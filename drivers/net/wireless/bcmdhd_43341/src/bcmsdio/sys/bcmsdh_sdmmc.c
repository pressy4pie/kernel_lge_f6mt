 ...
 *
 * $Id: bcmsdh_sdmmc.c 390090 2013-03-09 04:03:41Z $
 */
#include <typedefs.h>
                     ...
                     uint addr, uint8 *buf, uint len)
{
	...
	/* if the length is smaller than 32, or if it is a read, adjust the
	 * the length so it will be 4-byte aligned,
	 *
	 * NOTE: For all writes, each packet length is aligned to 32 (or 4)
	...
	uint addr, uint reg_width, uint buf_len, uint8 *buffer, void *pkt)
{
	...
	/* buffer is aligned, use it directly so we can avoid memory copy */
	if (((uint)buffer & DMA_ALIGN_MASK) == 0)
